...
python .\RetinaNetTraining\training_retinanet.py \
    -train_path="RetinaNetTraining/Datasets/ProspectiveData/prostate_Combined/train_0" \
    -val_path="RetinaNetTraining/Datasets/ProspectiveData/prostate_Combined/val_0" \
    -save_path="RetinaNetTraining/Model Results/prostate_Combined/fold_0" \
    -epochs=10 \
    -image_size=640 \
    -batch_size=2 \
    -patience=100
...
# Set up datasets.
########################################################################################################################
train_dataset = ProstateDataset(root=train_path, transforms=train_transforms)
...
print('=====================================================================================================\n'
      ...
      f'Total training images in dataset: {train_dataset.__len__()}.\n'
...
lr_schedular = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimiser, T_0=200, eta_min=0)
...
for epoch in range(total_epochs):
    ...
    ####################################################################################################################
    utils.plot_losses(training_losses, training_cls_losses, training_bbox_losses,
...
########################################################################################################################
# On training complete, pass through validation images and plot them.
########################################################################################################################
custom_model.model.eval()
with torch.no_grad():
    counter = 0
